...
\begin{document}
	\begin{figure}[!h]
		...
		\begin{tikzpicture}[
			...
			\draw[->, thick] (3.7,-2) -- (4.7,-2) node [midway, above] {L};
			
			\node (t2)[right = 8 cm of t1] {$t$}
			child {
				node (R2) [CircNode]{R}
				child{
					node (T2) [CircNode] {T}
			...
			\draw[->, thick] (t2) -- (R2);
			\draw[->, thick] (R2) -- (T2);
			\draw[->, thick] (T2) -- (6.1,-2.6);
			\draw[->, thick] (T2) -- (7.65,-2.6);
			\draw[->, thick] (R2) -- (9.9,-2.1);
			\node [right = 0mm of T2] {\texttt{0}};
			\node [right = 0mm of R2] {\texttt{0}};
			\node [below = 0mm of a2] {$h$};
			\node [below = 0mm of b2] {$h$};
			\node [below = 0mm of c2] {$h+1$};
			\draw[<->, thick] (11, -3.9) -- node [anchor=center, right, text width= 1.5cm, text centered]{$h(t)=$\\$h+2$}(11, 0.1);
		\end{tikzpicture}
